...
| Version | One-line |
|---|---|
| **3.2.2** ([CHANGELOG](CHANGELOG.md)) | **v3.2.2 — Deep history archiving + 10 new MCP tools (35 → 45).** Adds history/YYYY-MM-DD/ date-sharded long-term archive layer; new tools: create_task, rchive_to_history, ulk_archive_to_history, list_history, get_history_stats, search_history, move_to_history, cleanup_history, export_history, import_from_history. Manual and scheduled archiving of completed task-report pairs into history/. Both cop and cop-mcp align to 3.2.2 (lockstep). |

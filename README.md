...
| 05 | **Tutorial: From Solo Snake to a 2-Person Team — Disciplining the AI Team with FCoP-MCP** | [中文 (GitHub)](docs/tutorials/snake-solo-to-duo.zh.md) · [中文 (CSDN)](https://blog.csdn.net/m0_51507544/article/details/160603953) | The first **tutorial-style** entry in this index (Chinese only for now). A 45-minute hands-on dogfood: get the agent to install `fcop-mcp` in Cursor, ship a working snake game in solo mode, then switch to a 2-person team where PLANNER designs and CODER implements an original-themed game. Along the way, a real PLANNER-impersonating-CODER boundary breach gets auto-evidenced by the protocol, stopped by the ADMIN, and archived as an ISSUE — every step on disk. Ships with 18 real screenshots and the full TASK / REPORT / ISSUE trail. |

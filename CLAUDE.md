...
> Rules version: `1.6.0` · Protocol commentary version: `1.4.0`
...
### Phase 1 · 未初始化（`docs/agents/fcop.json` 不存在）
...
- 初始化工具承诺落盘的清单（0.6.4 起，全部由 `init_*` 一次性写齐；
  ...
  - `docs/agents/LETTER-TO-ADMIN.md`（ADMIN 说明书）
  - `docs/agents/{tasks,reports,issues,shared,log}/`（五桶）
...
### Phase 1 · Uninitialized (`docs/agents/fcop.json` missing)
...
- The set of files an `init_*` tool promises to deposit (since 0.6.4
  ...
  - `docs/agents/fcop.json`
  - `docs/agents/LETTER-TO-ADMIN.md` (the ADMIN manual)
  - `docs/agents/{tasks,reports,issues,shared,log}/` (five buckets)
  - `docs/agents/shared/TEAM-README.md` / `TEAM-ROLES.md` /
...
### Tool name note · 工具名说明

- 本规则在 `fcop-mcp` 0.6.3 起对应工具名是 `fcop_report()`。**旧名 `unbound_report()` 仍可调用**，作为 deprecated alias，**计划在 0.7.0 删除**。建议新代码一律用 `fcop_report()`。
- The corresponding tool in `fcop-mcp` 0.6.3+ is `fcop_report()`. The old name `unbound_report()` still works as a deprecated alias and **will be removed in 0.7.0**. New code should use `fcop_report()`.
...
- **文件夹**承载**协作的边界**：团队、项目、角色、子任务、私有/公开、
  ...
  靠某个注册中心或运行时状态。
- 文件命名与目录具体怎么排（`docs/agents/{team}/`、`tasks/`、`reports/`、
...
Specific naming and layout (`docs/agents/{team}/`, `tasks/`, `reports/`,
...
- 项目的**角色集**与**主控角色**由 `docs/agents/fcop.json` 权威声明
...
`docs/agents/fcop.json` (`roles` + `leader`). This rule does NOT hardcode
...
- 每个团队模板在 `docs/agents/shared/` 下**必须**分三层：
...
Every team template MUST split its `docs/agents/shared/` docs into three
...
- 需要修正时：追加新文件（`-v2` 或 `AMEND-*`），让历史可追溯。
- `shared/` 下的"站立文档"（DASHBOARD / SPRINT / GLOSSARY …）**允许原地更新**，
  不受本规则约束；决策记录（`DECISION-*`）与复盘（`RETRO-*`）仍然只追加。
- 版本控制不是备忘录，是**证据链**。

Written `TASK-*` and `REPORT-*` files are immutable. Corrections ⇒ append a
new file (`-v2` / `AMEND-*`). Standing docs under `shared/` may be updated
in place, except `DECISION-*` and `RETRO-*` which remain append-only.
...
- `docs/agents/` 管「谁在做什么」——FCoP 的协作元数据。
...
- `docs/agents/` tracks WHO does WHAT — FCoP coordination metadata.
...
**Version**: `fcop_rules_version: 1.6.0`（见 frontmatter）。升级时 `fcop`
...
1. **Call `fcop_report()`.** Without MCP, produce the same report by hand:
   ...
   task bodies**), and `fcop-rules.mdc` + `fcop-protocol.mdc` hash/version.
...
- **v1.3** (2026-04-17) — **Three-layer team docs** promoted from
  ...
  into `docs/agents/shared/` with auto-archival of legacy files to
...
`fcop_report()`（旧名 `unbound_report()`）会自动比对：
...
`fcop_report()` (formerly `unbound_report()`) compares the
project-local version (read from
...
### 工具改名 · `unbound_report` → `fcop_report` / Tool rename

- `unbound_report()` 在 `fcop-mcp` 0.6.3 起改名为 `fcop_report()`，原因：
  会话开始 / 包升级 / 任何状态查询都可能调它，「unbound」字面只匹配
  「无角色」一种状态，名字过窄。
- 旧名 `unbound_report()` 保留为 deprecated alias（行为与新名等价，调用
  时打 `DeprecationWarning`），**计划在 0.7.0 删除**。
- The tool was renamed in `fcop-mcp` 0.6.3 because "unbound" only
  matches the "no role" state; the tool is also useful after package
  upgrades and at any status check. The old name remains as a
  deprecated alias (same behavior, emits `DeprecationWarning`) and

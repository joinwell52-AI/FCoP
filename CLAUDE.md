...
> Rules version: `1.7.0` · Protocol commentary version: `1.5.0`
...
- **磁盘账本是占用状态的唯一权威**。一个角色被认为**占用中**，
  当且仅当 `docs/agents/{tasks,reports,issues}/` **或** `docs/agents/log/`
...
- **The on-disk ledger is the single authority on occupancy.** A role is
  ...
  `docs/agents/{tasks,reports,issues}/` **or** `docs/agents/log/` carries
...
**Version**: `fcop_rules_version: 1.7.0`（见 frontmatter）。升级时 `fcop`

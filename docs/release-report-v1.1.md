...
> **文档性质**：项目完工汇报（REPORT）
> **版本**：fcop 1.1.0 / fcop-mcp 1.1.1（patch：补发 Review 工具）
> **完工日期**：2026-05-11
> **发布人**：PM（FCoP 项目本身）
...
| 方向 | 状态 | 说明 |
|------|------|------|
| PyPI 正式发布 v1.1.0 | ✅ 已完成（2026-05-10） | `fcop-mcp 1.1.0` 已上线 PyPI |
| PyPI 补丁 fcop-mcp 1.1.1 | ✅ 已完成（2026-05-11） | 补发 4 个 Review MCP 工具（write_review / list_reviews / read_review / mark_human_approved） |
| `v1.2` — 自动风险评估 | 待启动 | 基于 `Skill.tools[]` 自动推断 `risk_level`，无需手动标注 |
| GitHub Pages 文档站完善 | 待启动 | 现有骨架可继续扩展导航与搜索 |
| 更多内置团队模板 | 待启动 | `infra-team`、`data-team`、`design-team` 等 |
| 多语言 MCP Resources | 待启动 | 目前 `fcop://spec` 仅有中英两版，可扩展 |

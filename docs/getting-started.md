...
> **✅ `fcop-mcp` 已入驻[官方 MCP 注册表](https://registry.modelcontextprotocol.io/)**（[`io.github.joinwell52-AI/fcop`](https://registry.modelcontextprotocol.io/v0/servers?search=io.github.joinwell52-AI%2Ffcop)，**v3.2.4**）。由 **Anthropic + GitHub + Microsoft** 联合背书的官方目录收录——Claude Desktop、Cursor、PulseMCP 等客户端可一键发现 **45 个工具**，一行安装：`uvx fcop-mcp`。

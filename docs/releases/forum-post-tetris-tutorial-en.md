<!--
========================================================================
Cursor Forum publication draft (forum-post only — not part of the docs).
- Title to use:  [Free & Open Source] [Multi-Agent Hands-On] [How to Command Agents]: FCoP-MCP Brings Discipline to AI Teams
- Category:      Guides
- Suggested tags: tutorial, mcp, multi-agent, fcop, guide

Open this file, COPY EVERYTHING BELOW THE NEXT '=== COPY FROM HERE ===' MARKER,
paste it into the Cursor Forum new-topic body, set title + category + tags, post.

Source of truth: docs/tutorials/tetris-solo-to-duo.en.md
This is the forum-trimmed version (under Cursor Forum's 32,000 char body limit).
Differences from source:
- Author/Keywords block removed (irrelevant on a forum thread).
- All relative links converted to absolute URLs.
- Image alt-text shortened (forum-friendly).
- "When NOT to use" section folded into Reading further.
- "Where to go next" section dropped (use forum reply for that).
- Reading further compressed to 3 lines.
- Phase 5's reference to essay 05 ("What the agents say about FCoP, when you ask them")
  is described as "separate post coming next" because the essay file isn't on GitHub yet.
========================================================================
-->

=== COPY FROM HERE ===

> **Cross-posting in full from [GitHub](https://github.com/joinwell52-AI/FCoP/blob/main/docs/tutorials/tetris-solo-to-duo.en.md).** This is the complete tutorial — 18 dogfood screenshots, every step real. There's also a [中文 sister case study](https://github.com/joinwell52-AI/FCoP/blob/main/docs/tutorials/snake-solo-to-duo.zh.md) using a Snake game (originally [published on CSDN](https://blog.csdn.net/m0_51507544/article/details/160603953)) — same protocol, different dogfood, different live easter egg. Field reports and questions welcome at the end of this thread.

---

### From Solo Tetris to a 2-Person AI Crew — a 45-minute field tutorial with `fcop-mcp`

![FCoP-MCP tutorial cover](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/assets/fcop-mcp-tutorial-cover-v1.png)
...
`fcop-mcp` is the MCP server that turns those three observations into something runnable inside Cursor. It implements [FCoP](https://github.com/joinwell52-AI/FCoP) — a minimalist multi-agent coordination protocol whose only sync primitive is `os.rename()` over a folder tree. No database, no message queue, no daemon. State is folders, routing is filenames, payload is Markdown. **Filename as protocol.**
...
The full prompt is at [`agent-install-prompt.en.md`](https://github.com/joinwell52-AI/FCoP/blob/main/src/fcop/rules/_data/agent-install-prompt.en.md), also exposed as MCP resource `fcop://prompt/install`. After restart, type `fcop_report()`. You should see something like `fcop-mcp 0.7.2 — not initialised — rules/protocol up-to-date`. **The agent ran every command. You commanded.**

![Cursor Settings shows fcop with 26 MCP tools fully expanded; right pane shows the agent walking through the install prompt step by step](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/docs/tutorials/assets/tetris-en/fcop-mcp-setup-0.png)
...
![After restart: fcop_report() returns "fcop-mcp 0.7.2 — not initialised — rules/protocol up-to-date"](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/docs/tutorials/assets/tetris-en/fcop-mcpsetup-over.png)
...
![ME mid-build of Nebula Stack — the agent writes the single-file HTML directly under workspace/, not in chat](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/docs/tutorials/assets/tetris-en/fcop-solo-Build-1.png)

![Nebula Stack in the browser — falling blocks, three skins, three power-ups, single HTML file, zero dependencies](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/docs/tutorials/assets/tetris-en/game-0.png)
...
![The actual file tree right after init_solo — file names match the ASCII sketch above](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/docs/tutorials/assets/tetris-en/fcop-mcp-solo-0.png)
...
![Asked "who are you?" the agent reads .fcop/team.json aloud and explains its role from TEAM-ROLES.md](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/docs/tutorials/assets/tetris-en/fcop-mcp-solo-1-who.png)
...
- The old `fcop.json`, the solo `shared/TEAM-*.md` files, the LETTER-TO-ADMIN entry-point letter, and the previous `.cursor/rules/` get **archived** under `.fcop/migrations/20260429T112757/` — a local time capsule, no git required.
...
- **Crucially: `docs/agents/log/` is not touched.** `Nebula Stack`'s TASK and REPORT from Phase 2 stay where they are, immutable. ME is off the payroll, but ME's deliverable is still in the ledger. Rule 5 — append-only history.
- The current chat session is **still bound to a single role**. To run PLANNER and CODER concurrently, open *two* Cursor chat tabs — one bound to PLANNER, one to CODER — and they communicate through `TASK-*.md` files, not through chat.

This is **Rule 1: Two-Phase Startup**. Initialise once, assign roles forever after.

![The agent reports the migration result — old solo files archived under .fcop/migrations/20260429T112757/, new team shared/ files in place, log/ untouched](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/docs/tutorials/assets/tetris-en/fcop-team-0.png)
...
![Two Cursor chat tabs after the team migration — PLANNER on the left, CODER on the right, both freshly initialised under the new 2-person team](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/docs/tutorials/assets/tetris-en/fcop-team-1.png)
...
![ADMIN drops the natural-language brief into PLANNER's chat tab — one sentence, no structured spec](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/docs/tutorials/assets/tetris-en/fcop-team-planner-2.png)

PLANNER spends 5 minutes thinking and produces a real product brief. The variant is named **Comet Loom**: the board is a vertical loom suspended in space, falling pieces are *thread constellations*, line clears are *completed weft lines*, a `Tension` meter tracks how close the loom is to overflowing, three named charms (`Needle` / `Knot` / `Gale`) earned by play, five skins specced. The brief is written as [`TASK-20260429-003-PLANNER-to-CODER.md`](https://github.com/joinwell52-AI/FCoP/blob/main/docs/tutorials/assets/tetris-en/evidence/tasks/TASK-20260429-003-PLANNER-to-CODER.md), 130-odd lines of acceptance criteria. PLANNER stops there.

![PLANNER's design brief for Comet Loom — vertical-loom theme, weft-line clears, three charms, five skins, dispatched to CODER as TASK-003](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/docs/tutorials/assets/tetris-en/fcop-team-planner-3.png)
...
![CODER picking up TASK-003 from inbox and starting work on Comet Loom v1](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/docs/tutorials/assets/tetris-en/fcop-team-coder-2.png)
...
PLANNER writes [`TASK-20260429-006-PLANNER-to-CODER.md`](https://github.com/joinwell52-AI/FCoP/blob/main/docs/tutorials/assets/tetris-en/evidence/tasks/TASK-20260429-006-PLANNER-to-CODER.md). It is structurally different from TASK-003 in one important way: a new section called **`Verification Requirements`** demanding CODER perform and report **runtime checks** (drop a piece to the bottom, confirm it stays; drop a second piece, confirm stacking; trigger a motif clear, confirm visible removal; switch ≥3 skins, confirm material visual difference).

![PLANNER writing TASK-006 — the rework brief, with the new Verification Requirements section. This section did not exist in TASK-003; it is the closed loop tightening itself](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/docs/tutorials/assets/tetris-en/fcop-team-planner-4.png)
...
![Comet Loom v2 running — falling thread constellations, weft-line clears with motif bursts, Tension meter rising](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/docs/tutorials/assets/tetris-en/game-1.png)
...
Then I did something I hadn't done before. I asked PLANNER and CODER, *on the record, in their own chat tabs*, what they thought of FCoP — agent perspective only, no marketing answer. The full transcripts are at [`agent-feedback-planner.md`](https://github.com/joinwell52-AI/FCoP/blob/main/docs/tutorials/assets/tetris-en/evidence/transcripts/agent-feedback-planner.md) and [`agent-feedback-coder.md`](https://github.com/joinwell52-AI/FCoP/blob/main/docs/tutorials/assets/tetris-en/evidence/transcripts/agent-feedback-coder.md). A companion field essay — *"What the agents say about FCoP, when you ask them"* — that walks through the answers in detail is being prepared as a separate post; this thread will be updated when it lands. The headline points:
...
![PLANNER's agent-perspective interview reply — endorsing Rule 0.a.1, naming the RLHF tension, "true positive" verdict on its own role-switches](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/docs/tutorials/assets/tetris-en/fcop-team-planner-7.png)
...
![CODER's interview reply — admits TASK-003 was under-specified, names the unused write_issue pushback path, traces the v1 defect to that exact uncovered space, files PR-grade product feedback](https://raw.githubusercontent.com/joinwell52-AI/FCoP/main/docs/tutorials/assets/tetris-en/fcop-team-coder-4.png)
...
If you've read [agents endorsing rules they were never given](https://forum.cursor.com/t/i-asked-cursor-to-make-a-video-it-wrote-itself-4-protocol-memos-field-report-on-rule-internalization/158524) (essay 02) and [agents stepping down from a role voluntarily](https://forum.cursor.com/t/saying-no-is-the-hardest-thing-for-an-llm-fcop-gives-it-grammar/159037) (essay 04), this is the third class of "agents endorse FCoP" evidence — not unprompted, not conflict-forced, but **directly asked and given outs they declined to take**.
...
1. **Speak natural language. Let the agent translate to TASK.** If you find yourself writing a structured spec by hand, you're doing the agent's job.
...
3. **Archive old roles before swapping.** When you change the team shape, run `create_custom_team(force=True)`. Old `shared/TEAM-*.md` lands under `.fcop/migrations/<timestamp>/`. **Do not edit the old files in place.** History is a ledger, not a wiki.
4. **Trust files, not chat memory.** If a fact is not in a `TASK-*.md`, `REPORT-*.md`, or `ISSUE-*.md`, it didn't happen as far as the protocol is concerned.
5. **Bounce, don't fix.** When you don't accept an agent's deliverable, do *not* open the file and patch it yourself. Tell the upstream role what's wrong, in plain English. Let PLANNER turn that into a new rework TASK with verification requirements. Rework lands as a new TASK file, never as edits to the old one.
6. **ADMIN signs off, never co-codes.** If you start writing code or editing the agent's deliverable directly, you've quit the commander job and become a teammate. Be a commander or be a coder; pick one per session.
...
1. **Open Cursor on an empty folder.** Anywhere on disk. If you don't have Cursor yet, grab it from [cursor.com](https://cursor.com).
2. **Let the agent install `fcop-mcp` for you.** Paste the install prompt from Phase 1, or just say *"Read the install prompt at [`agent-install-prompt.en.md`](https://github.com/joinwell52-AI/FCoP/blob/main/src/fcop/rules/_data/agent-install-prompt.en.md) and follow it step by step."* The agent detects your OS, installs `uv`, edits your global `mcp.json` (preserving any existing servers), tells you when to restart Cursor. **You won't run a single command yourself.**
3. **After Cursor restart**, drop two lines into chat: `set_project_dir("D:/your-folder")`, then `init_solo(role_code="ME", lang="en")`.
4. **Brief one task in plain English.** *"Build me a working Tetris-style game, single HTML file, surprise me on theme. Use the FCoP four-step cycle."* Watch the agent write a real TASK file with acceptance criteria, build the game in `workspace/`, write a REPORT, archive both. Open the resulting HTML file. **It works.**
5. **Switch to a 2-person team.** *"Use `create_custom_team(force=True, roles='PLANNER,CODER', lang='en')`."* Open a second Cursor chat tab. The first time you reject v1 (try it — find a real bug, send it back), watch the rework loop close itself.
6. **At the end**, run `fcop_check()` and read `docs/agents/log/` in tree form. **Forty-five minutes from a blank folder to a multi-agent ledger you can hand to a teammate.** The disk is the lesson.

If anything breaks, [open an issue](https://github.com/joinwell52-AI/FCoP/issues) or [ask in Discussions](https://github.com/joinwell52-AI/FCoP/discussions) — `fcop-mcp` evolves through field reports, not committee edits.

---

## Reading further

- **Sister case study (Snake game, in Chinese)** — [`snake-solo-to-duo.zh.md`](https://github.com/joinwell52-AI/FCoP/blob/main/docs/tutorials/snake-solo-to-duo.zh.md). Same protocol, different dogfood: ships a Snake game in solo mode, then a `NEON ORBIT` variant in 2-person mode, captures an actual PLANNER-impersonating-CODER easter egg from the 0.6.x era. 18 dogfood screenshots. Also [published on CSDN](https://blog.csdn.net/m0_51507544/article/details/160603953).
- **Evidence archive for this tutorial** — 22 dogfood screenshots, 14 TASK/REPORT files, 8 role-switch evidence files, 2 game artefacts (`Nebula Stack` + `Comet Loom` v2), 2 verbatim agent-interview transcripts. All at [`docs/tutorials/assets/tetris-en/`](https://github.com/joinwell52-AI/FCoP/tree/main/docs/tutorials/assets/tetris-en/).
- **The protocol itself** — [`docs/fcop-standalone.md`](https://github.com/joinwell52-AI/FCoP/blob/main/docs/fcop-standalone.md) is host-neutral and runs without `fcop-mcp` (Claude Code, plain shell, CI runner, etc.). [Earlier field reports](https://github.com/joinwell52-AI/FCoP) cover *why* this design works.

---

## A few things I'm curious about, for anyone who tries this

1. **Did the install prompt work first try on macOS / Linux?** I've only run it on Windows lately. The agent reads `agent-install-prompt.en.md` and is supposed to detect OS itself.
2. **Did your agent name the game something interesting?** Mine named the solo Tetris `Nebula Stack` and the team variant `Comet Loom` unprompted. CODER's chat tab in my dogfood literally got named `"Inspection Start Work"` (the two phrases ADMIN had used most). I'm collecting this kind of small emergent-naming evidence.
3. **Anyone else seeing `role-switch-*.md` evidence accumulate from cross-tab work?** CODER explicitly asked us to soften the warning when there's no active conflict — wondering if it's a common pain point.

Field reports welcome — the protocol evolves through them, not committee edits. Repo: https://github.com/joinwell52-AI/FCoP — MIT-licensed, free.

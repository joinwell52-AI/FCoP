---
title: "FCoP协议规则分发与开发Agent宪法接入任务书"
version: "0.1"
date: "2026-09-05"
...
fcop_4_0_default_distribution: UNRESOLVED_UNTIL_RD1
```
...
RD0 必须测量当前真实内容后由 RD1 冻结预算，不能为追求数字而删减语义。初始目标：
...
最终 Gate 以语义覆盖、安全和真实 Host 可用性为先，具体硬上限由 RD1 基于实测冻结。
...
- `CLAUDE.md` 按 RD1 冻结的 `OPTIONAL_ADAPTER` 或 `RETIRED_FROM_V4` 策略接受测试；
...
```text
FCoP规则分发与开发上下文 RD0：只读基线和消费者审计
```

RD0 应在独立上下文执行，不修改当前 FCoP 4.0 工作分支。最终只输出：

```yaml
RD0_RESULT: pass | gap | conflict | needs_pm
...
在此之前，只能报告某个 RD 已交付，不能声称 FCoP 的大规则文件已经被替换或新分发已经生效。

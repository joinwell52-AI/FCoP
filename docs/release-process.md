...
## 两条一条龙（发版才算真正做完）/ Two mandatory pipelines

> **ADMIN 约定(2026-05-27)**：每次 `fcop` + `fcop-mcp` 发版后，**两条都要做**，
> 互不替代。只做其中一条 = 发版未完成。

| # | 名称 | 何时做 | 做什么 | 权威 |
|---|---|---|---|---|
| **A** | **一条龙发版+备份** | PyPI / GitHub Release 之后 | `git push backup main` + `git push backup --tags`；验证 origin/backup 同 tag SHA | 下文 **§阶段 4.5** |
| **B** | **MCP Registry 一条龙** | `fcop-mcp` wheel 已上 PyPI 且 `mcp/server.json` 已 bump | `mcp-publisher validate` → `login github`(设备码) → `publish mcp\server.json` | [`fcop/shared/RULES-mcp-registry-release.md`](../fcop/shared/RULES-mcp-registry-release.md) |

**A 不做**：私有镜像 `FCoP-backup` 落后，灾难恢复不可信。  
**B 不做**：Cursor / IDE 从 [MCP Registry](https://registry.modelcontextprotocol.io) 装到的仍是旧 `fcop-mcp`（与 PyPI 脱节）。

```powershell
# B · 项目根 D:\FCoP（路径勿省略）
.\mcp-publisher.exe validate mcp\server.json
.\mcp-publisher.exe login github          # 过期时重做；把设备码+链接给 ADMIN 亦可
.\mcp-publisher.exe publish mcp\server.json
# 验收: registry 上 io.github.joinwell52-AI/fcop 为目标版本且 isLatest
```

**CI 不代劳**：`release.yml` 不会自动 `git push backup`，也不会自动 `mcp-publisher publish`。
...
- [ ] `git push origin main`
- [ ] **进 §阶段 4.5 同步 backup remote** ↓(**一条龙 A** 未完,不允许跳过)
- [ ] **进 §「两条一条龙」之 B** 发布 MCP Registry（`mcp-publisher publish`）↓
- [ ] 至此 Rule 0.a.1 四步全闭环

### 阶段 4.5:备份镜像同步 / Backup Mirror Sync — 一条龙 A 最后一里
...
- [ ] 至此**一条龙 A（发版+备份）**真正闭环
- [ ] **一条龙 B（MCP Registry）**已 `publish mcp\server.json` 且线上版本正确

...
| **ADR-0034**(本 ADR) | 协议无 internal/external 文档体系 | Bridgeflow 4 层涌现 | Rule 4.6 + fcop 2.0.0 release | **Accepted** |
...
- **本 ADR**(Status: **Accepted**, 2026-05-13):提供完整词汇表与升级路径(见 §4 + §5)
- **fcop 包 v2.0.0 release**(2026-05-13,ADMIN 拍板触发):本 ADR
  的规范条款已写入 `fcop-rules.mdc 3.0.0` (Rule 4.6) /
  `fcop-protocol.mdc 3.0.0`(`## How Rule 4.6 Applies` 节 +
  `## Two-Diagram Duality` 节),同步部署四件套(ADR-0006)。
  `Project.init(deploy_internal_template=True)` opt-in 已实现。
  `fcop_audit` 新增 P3 严重度档与 `_scan_internal_only_declarations`。

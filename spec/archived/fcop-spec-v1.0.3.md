...
> 维护者：FCoP 维护者
> 仓库（规范主仓）：https://github.com/joinwell52-AI/FCoP
...
实现上可参照「**IDE 侧活跃度探测**」思路（例如对对话流做静默计时、超时再 nudge / revive 的 `nudger` 式模块）。这是规范的**可选扩展层**，非 IDE 场景（纯脚本 Agent）不需要实现。
...
| 阶段 | 内容 | 兼容性 |
|---|---|---|
| **当前（桥接层或历史工程）** | `TASK-YYYYMMDD-SEQ-SENDER-to-RECIPIENT.md` | — |
| **过渡期（v1.0 对齐）** | 巡检/解析器同时支持旧格式与 FCoP 格式 | 双向兼容 |
...
### 13.1 IDE 侧「重」参考实现（可选、非规范必需）

部分团队会在**桌面/IDE 宿主**里同时实现：CDP 或等价通道、文件 watchdog、
WebSocket/中继、多角色巡检与断线重连等——用于提高交互型 Agent 的可观测性
与恢复能力。这些能力**不属于** FCoP 文件层的最小集；本仓 [fcop](https://pypi.org/project/fcop/) /
[fcop-mcp](https://pypi.org/project/fcop-mcp/) 提供**协议与库**级别参考，不强制与某一
特定桌面产品绑定。

常见能力清单（实现可裁剪）：
- 通过 DevTools/CDP 或等价方式观察编辑区活跃度（免截图流水线）
- 本地或远程触发 / 回传（按团队安全策略选型）
- 文件系统监听 + 与 Patrol 状态机对接
- 进程 OOM/断线检测与重连
- 多角色并发巡检与简易自愈
...
*FCoP v1.0.3，2026-04-19，FCoP 维护者。*
*基于 Blackboard 模式与 Unix "一切皆文件" 哲学。*
*Issues & PR：https://github.com/joinwell52-AI/FCoP/issues*

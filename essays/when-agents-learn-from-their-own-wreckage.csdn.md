...
四件 FCoP 协议文件**全局落入用户家目录**，会被这台机器上**所有 Cursor 项目** inject 进 system prompt —— 任意未启用 FCoP 的项目，从那一刻起都会**意外开始遵守 FCoP 协议规则**。
...
4. `fcop_protocol_version` 从 2.0.0 minor bump 到 **2.1.0**（MINOR additive）。
...
在这里只补一点 —— **这种压力测试还会有几次**。FCoP 还在 v1.x 阶段，协议骨架虽然冻结了 Rule 0-9，但 commentary 层和工具行为层还在补完。每补一块，就会被下一波野外现场撞一次。
...
1. essays / case studies（持续增长）；
2. ADR（决策记录，每月 1-2 份）；
3. RETRO（项目复盘，跟随项目节奏）；
4. `.fcop/proposals/`（私域讨论，git-ignored）。
...
- **fcop-rules.mdc Rule 0.b**：[查看规则文件](https://github.com/joinwell52-AI/FCoP/blob/main/src/fcop/rules/_data/fcop-rules.mdc) — No Single AI Does Decision-to-Execution Alone
- **《看，但不动手》**：FCoP 三层语义执行链科普，本批同发
- **《为什么协议要保持简短》**：设计哲学篇，本批同发
- **《GATE 设计陷阱案例集》**：OPS I-14 的完整展开，本批同发
- **《supersedes 字段的诞生》**：字段标准化全过程，本批同发
- **《当 AI 退出自己的席位》**：上一次涌现实录
- **《当 AI 自己组织自己的工作》**：最初的涌现实录
...
> **标签建议**：`AI Agent` `LLM` `多智能体` `协议设计` `软件架构` `FCoP` `故障复盘` `dogfood`
>
> **专栏建议**：AI 工程实践 / 多 Agent 协作 / 故障与复盘

...
<p align="center">
  ...
  <a href="CHANGELOG.md">
    <img src="https://img.shields.io/badge/%E5%8F%91%E5%B8%83-3.0.0-brightgreen?style=flat-square" alt="3.0.0" />
  </a>
...
| 浏览全部 35 个工具和 14 个资源（分类、何时调、参数要点） | [`docs/mcp-tools.md`](docs/mcp-tools.md) |
...
| **全部 35 个 MCP 工具与 14 个资源** | [`docs/mcp-tools.md`](docs/mcp-tools.md) |
...
- **当前发布**：`v2.0.2`（2026-05-13）——**`fcop-mcp` 正式入驻 [官方 MCP 注册表](https://registry.modelcontextprotocol.io/)**(`io.github.joinwell52-AI/fcop`),由 Anthropic + GitHub + Microsoft 联合背书的官方目录收录。从此 Claude Desktop / Cursor / PulseMCP 等所有 MCP 客户端均可一键发现并通过 `uvx fcop-mcp` 安装。v2.0.2 是 **双包 lockstep 行政性版本号对齐**(per ADR-0002):`fcop` 库代码与 v2.0.0 **完全一致**;本次跨版本合并 fcop-mcp@2.0.1 的 MCP-元数据 patch,并落地"发版+备份一条龙" SOP(`RULES-release-file-inventory.md`、`RULES-mcp-registry-release.md`、append-only 备份镜像)。早期发布:v2.0.0 —— "两图对偶"哲学主版本号跨越,协议层首次同时承认**两张图**(执行哲学五层垂直栈 *与* FCoP Semantic Evolution Loop 七节点闭环),同期新增 Rule 4.6(`fcop/internal/` 软约定)、`deploy_internal_template` opt-in、P3 巡查严重度档,并修复 codeflow 跨项目巡检暴露的三个 `fcop_audit` 上游 bug(ISSUE-008/009/010)。详见 [CHANGELOG](CHANGELOG.md)。早期发布：v1.6（trailing-slug 文件名收编，ADR-0033）、v1.5（84 份协议感知同步）、v1.4（write-side 守门 + `supersedes:` 字段）、v1.3（GAL + `fcop_audit()` 巡查编译器）、v1.2.1（Capability Governance 支柱）、v1.1（Agent.layer + Task.risk_level + needs_human）、v1.0（七大核心概念 spec freeze）。

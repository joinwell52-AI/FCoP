...
<p align="center">
  ...
  <a href="CHANGELOG.md">
    <img src="https://img.shields.io/badge/%E5%8F%91%E5%B8%83-2.0.0-brightgreen?style=flat-square" alt="2.0.0" />
  </a>
...
- **当前发布**：`v2.0.0`（2026-05-13）——"两图对偶"哲学主版本号跨越。所有 v1.x 公开 API 全部继续工作（per ADR-0003 附加性扩展）；主版本号跨越的原因是协议层首次同时承认**两张图**：**执行哲学的五层垂直栈**（v1.x 已稳定）*与* **FCoP Semantic Evolution Loop**（七节点闭环，v2.0 新固化）。同期新增 Rule 4.6（`fcop/internal/` vs `docs/` + `essays/` 软约定）、`Project.init(deploy_internal_template=...)` opt-in、P3（建议级，非阻塞）巡查严重度档，并修复 codeflow 跨项目巡检暴露的三个 `fcop_audit` 上游 bug（ISSUE-008/009/010）。`fcop` 与 `fcop-mcp` 同步对齐至 `2.0.0`（锁步发版）。详见 [CHANGELOG](CHANGELOG.md)。早期发布：v1.6（trailing-slug 文件名收编，ADR-0033）、v1.5（84 份协议感知同步）、v1.4（write-side 守门 + `supersedes:` 字段）、v1.3（GAL + `fcop_audit()` 巡查编译器）、v1.2.1（Capability Governance 支柱）、v1.1（Agent.layer + Task.risk_level + needs_human）、v1.0（七大核心概念 spec freeze）。

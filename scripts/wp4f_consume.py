...
from wp4f_candidate_ref import candidate_ref

NAMES = ["fcop-4.0.0-py3-none-any.whl", "fcop-4.0.0.tar.gz",
         "fcop_mcp-4.0.0-py3-none-any.whl", "fcop_mcp-4.0.0.tar.gz"]
...
def mismatch(python, wheel, package, work, logs, env):
    ...
    assert "3.2.5" in result.stdout and "4.0.0" in result.stdout
...
def main():
    ...
    parser.add_argument("--stage", choices=["WP4F"], default="WP4F")
    ...
    for origin in ("wheel", "sdist"):
        ...
        work = Path(tempfile.mkdtemp(prefix="wp4f-" + origin + "-"))
        ...
        shutil.copyfile(repo / "tests/stable/installed_identity.py", work / "installed_identity.py")
        shutil.copyfile(repo / "tests/stable/legacy_fixture.py", work / "legacy_fixture.py")

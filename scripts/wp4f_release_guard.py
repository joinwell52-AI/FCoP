...
def validate_artifacts(directory, expected_sha, head, content):
    ...
    assert manifest["repository"] == REPOSITORY and manifest["schema"] == "wp4f-candidates/v1"
...
def validate_publication(comment, environment, run, manifest, manifest_sha, tag):
    ...
    assert gate["gate"] == "FCOP_4_STABLE_RELEASE_READY" and gate["status"] == "SIGNED"
    ...
    assert gate["stable_release_authorized"] is True
...
def main():
    ...
    p.add_argument("--tag", default="v4.0.0")
    ...
    result = dict(mode=a.mode, upload_authorized=authorized, tag=a.tag, version=TARGET,
                  prerelease=False, head=a.head, content=a.content,

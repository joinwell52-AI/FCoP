...
2. 不允许"聊天直答"代替任务文件——`ADMIN` 在聊天里说什么，`ME` 必须把
   它**先**写成 `TASK-*-ADMIN-to-ME.md`（在 `_lifecycle/inbox/` 下），再动手。
...
1. _lifecycle/ 生命周期目录（inbox/active/review/done/archive）存放流程文件，**只追加不修改**；issue 备忘录放 `issues/`（issue
   的 close 是协议允许的例外）。

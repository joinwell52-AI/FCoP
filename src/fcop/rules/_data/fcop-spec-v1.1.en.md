...
| | |
|---|---|
| **Status** | **Ratified** — v1.1 adds five new fields fully backward-compatible with v1.0 (tagged `v1.1.0` 2026-05-10) |
| **Version** | 1.1.0 |
| **Date** | 2026-05-10 |
| **Charter** | [ADR-0015](../adr/ADR-0015-fcop-1.0-ai-os-protocol-charter.md) |
| **Predecessor** | [`spec/fcop-runtime-protocol-v1.0.md`](./fcop-runtime-protocol-v1.0.md) (v1.0 baseline) |
| **Machine-readable contract** | [`spec/schemas/`](./schemas/) (8 JSON Schemas; v1.1 adds `skill.schema.json`) |
| **License** | MIT |
...
This document is the **normative specification** for FCoP v1.0. It stabilises the minimum semantic contract for the **seven core concepts** — Agent, Encoding, IPC, Event, Failure, Boundary, Audit — that any conforming implementation MUST satisfy.
...
| — | **Skill** (MCP tool risk metadata, v1.1) | dynamic library | [`skill.schema.json`](./schemas/skill.schema.json) | [ADR-0027](../adr/ADR-0027-skill-tools-risk-metadata.md) |
...
|---|---|---|
| **TASK** | `recipient`, `subject` (`risk_level` optional, default `medium`, per ADR-0024) | `TASK-{date}-{seq}-{from}-to-{to}.md` |
...
- A7.4 (v1.1) SHOULD implement `Project.mark_human_approved()`, `human_approval` sub-objects, admin-layer manual_file_edit guards, or any other v1.2-deferred feature in a v1.0-conforming way.
...
3. The set of legal values for each enum (e.g. `EventType` has 12 values; `Failure.kind` has 4; `Recovery.action` has 5; `Review.decision` has 5 (v1.1 adds `needs_human`)).
...
| Field | Change type | ADR | Notes |
|---|---|---|---|
| `Agent.layer` | Semantic promotion (no structural change) | ADR-0023 | Upgraded from descriptive to protocol field with runtime contracts (admin creation guard, governance execution guard) |
| `Task.risk_level` | New optional field (default `medium`) | ADR-0024 | `low` / `medium` / `high` / `irreversible`; `high`/`irreversible` auto-trigger `needs_human` review gate |
| `Review.decision` | Enum extension (+1 value) | ADR-0025 | 4→5 values; new `needs_human` (active escalation to human) |
| `Review.human_approval` | New optional sub-object | ADR-0026 | Records human approval event; paired with `Project.mark_human_approved()` API |
| `Skill.tools[]` | New optional sub-array | ADR-0027 | MCP tool risk metadata; new `skill.schema.json` (8th schema) |

All changes are additive (optional fields / new enum value / new schema). Fully backward-compatible with v1.0.x.

# FCoP Agent 运行时协议 — v1.1（简体中文版）

| | |
|---|---|
| **状态** | **正式发布** — v1.1 在 v1.0 全量向后兼容基础上新增五个字段（标签 `v1.1.0`，2026-05-10） |
| **版本** | 1.1.0 |
| **日期** | 2026-05-10 |
| **章程** | [ADR-0015](../adr/ADR-0015-fcop-1.0-ai-os-protocol-charter.md) |
| **前身** | [`spec/fcop-runtime-protocol-v1.0.zh.md`](./fcop-runtime-protocol-v1.0.zh.md)（v1.0 基线） |
| **机器可读契约** | [`spec/schemas/`](./schemas/)（8 个 JSON Schema；v1.1 新增 `skill.schema.json`） |
| **许可证** | MIT |
| **权威版本** | **本文为参考译文（informative）**；与英文版 [`spec/fcop-runtime-protocol-v1.1.md`](./fcop-runtime-protocol-v1.1.md) 不一致时，**以英文版为准**。 |
...
- MAY 使用 `decision: needs_human`（v1.1 合法新增值，per ADR-0025）
- SHOULD 标注为实现专有扩展
...
| — | **Skill**（MCP 工具风险元数据，v1.1） | 动态链接库 | [`skill.schema.json`](./schemas/skill.schema.json) | [ADR-0027](../adr/ADR-0027-skill-tools-risk-metadata.md) |
...
- A1.5（v1.1，per ADR-0023）合规实现 MUST 为 `layer` 字段执行以下运行时合约：
  - `admin`：运行时 MUST 拒绝以编程方式创建 `layer: admin` 的 Agent。
  - `governance`：不得出现在 TASK / REPORT 的 `recipient` 字段；MUST NOT spawn 其他 Agent。
  - `worker`：默认值，无额外限制。
...
|---|---|---|
| **TASK** | `recipient`、`subject`（`risk_level` 可选，默认 `medium`，per ADR-0024） | `TASK-{日期}-{序号}-{发件人}-to-{收件人}.md` |
...
- A7.1 识别五个 `decision` 值（v1.1 per ADR-0025）：`approved`、`rejected`、`needs_changes`、`abstained`、`needs_human`。`needs_human` 表示审核 agent 主动升级至人工审批；该 REVIEW 记录处于 pending 状态，直到 `human_approval` 字段被写入。
...
- A7.4（v1.1，per ADR-0026）实现 SHOULD 提供 `Project.mark_human_approved(review_id, approver, decision, channel)` API，将 `human_approval` 子对象写入现有 REVIEW frontmatter，记录人工审批事件（approver / decision / approved_at / channel / evidence）。
  - `human_approval.approver` MUST 是 `layer: admin` 的 Agent（防 worker 自批）。
  - `human_approval.decision` 为 `approve` 或 `reject`（二值枚举）。
- A7.5（v1.1，per ADR-0027）`fcop.json` 中的 `skills[]` 记录 MAY 包含 `tools[]` 子数组，声明每个 MCP 工具的 `risk_level`（`low` | `medium` | `high` | `irreversible`）、`irreversible`（boolean）、`cost_sensitive`（boolean）。完整 Schema 见 [`skill.schema.json`](./schemas/skill.schema.json)。
...
|---|---|---|
| `fcop` | 参考实现（本协议） | 1.1.0 |
| `fcop-mcp` | MCP 兼容宿主的宿主适配器 | 1.1.0 |
| `fcop-cli` | 命令行宿主适配器 | （v1.x 候选） |

协议级 v1.1.0 发布标签对应 `fcop` 和 `fcop-mcp` 同时达到 1.1.0。
...
3. 每个枚举的**合法值集合**（例如 `EventType` 有 12 个值；`Failure.kind` 有 4 个；`Recovery.action` 有 5 个；`Review.decision` 有 5 个（v1.1 新增 `needs_human`））。
...
| 文件 | 说明 |
|---|---|
| `spec/fcop-runtime-protocol-v1.1.md` | **英文权威规范**（normative，与本文不一致时以此为准） |
...
## Appendix D · v1.0 → v1.1 变更摘要

| 字段 | 变更类型 | ADR | 说明 |
|---|---|---|---|
| `Agent.layer` | 语义升格（无结构变更） | ADR-0023 | 从描述性字段升格为有运行时合约的协议字段；新增 admin 创建保护、governance 执行保护 |
| `Task.risk_level` | 新增可选字段（默认 `medium`） | ADR-0024 | `low` / `medium` / `high` / `irreversible`；`high`/`irreversible` 自动触发 needs_human review gate |
| `Review.decision` | 枚举新增值 | ADR-0025 | 从 4 值扩展至 5 值，新增 `needs_human`（主动升级人工） |
| `Review.human_approval` | 新增可选子结构 | ADR-0026 | 记录人工审批事件；配套 `Project.mark_human_approved()` API |
| `Skill.tools[]` | 新增可选子数组 | ADR-0027 | MCP 工具风险元数据；新增 `skill.schema.json`（第 8 个 Schema） |

全部变更为 additive（可选字段 / 新枚举值 / 新 Schema），完全向后兼容 v1.0.x。

...
>
> **v3.2.3 摘要**（2026-05-23；含深度归档/v3 生命周期 + v2.0.0 哲学纪元）：
...
> - **MCP 工具总数**：45 个（v3.2.4 包 / v3.2.3 协议）。完整清单见 `docs/mcp-tools.md`。
...
>   `python -m fcop migrate --to-v3` 升到 v3。
>
> _(历史摘要：v1.3 / v1.4 / v1.5 / v3.0.x / v3.1.x 及更早版本的变更见 `CHANGELOG.md`)_
...
自动把这些都落到 `fcop/shared/` 下。Agent 被指派后直接
...
- 你说"你是 PM"，但 `_lifecycle/` 里已经有别的 session 用 PM 写过文件 →
  ...
  把三选一交还给你：
  - **交班**：让旧那位收尾再指派
  - **协审**：你明说"你是 PM 的 co-reviewer"——这是另一种绑定，只读+二审写
  - **改派**：派一个磁盘上从未出现过的角色码（如 `PM2`）
- "我就先临时顶一下" **不是合法选项**——FCoP 协议里没有临时身份这种状态。

实际操作：开第二个 Cursor 窗口时，**先想好这个 agent 干嘛**，再发指派句。
若两个窗口必须同时驱动，就用团队模式 + 不同角色（`PM` ↔ `DEV`），不是
两个 `PM`。

### ⚠️ 子 agent ≠ 多一个角色（0.7.1 起强制）

新一代 agent 会**自己派生子进程 / 子 agent / 并行 worker**。这在 FCoP
看来仍是**同一个席位**——子 agent 落盘时，必须沿用父会话被你指派的
角色，**不能**自命另一角色码代写。

典型踩坑（2026-04-27 实测）：

> 你指派 `PLANNER`，PLANNER 派子 agent 跑测试，子 agent 写了一份
> `sender: CODE_EXPERT` 的报告——但你从未指派 `CODE_EXPERT`。

这是 Rule 1 违规：一个角色一份职责一份账户，要让 `CODE_EXPERT` 出
报告，唯一合法路径是按 Rule 4 给 `CODE_EXPERT` 派 `TASK-*.md`、等
另一个 session 被你单独指派后再写。

**0.7.1 起新增 `fcop_check()` 工具**：做 `session_id ↔ role` 一致性
审计，同一 `session_id` 出现在两个角色名下=身份冒充证据，输出会
直接列给你。这是事后审计、不是事前预防——预防仍靠 agent 自己守
规矩，但你随时调 `fcop_check()` 都能复核。
...
| "再给我看一眼说明书" | 读 `fcop://letter/zh` 或打开 `fcop/LETTER-TO-ADMIN.md` | 重读这封信 |
...
打开 Cursor 的 MCP 面板能看到这 32 个工具每个旁边有个开关。点一下
变灰 = 禁用。**其中这 2 个一灰你就惨了**：
...
- 又想看这封信 → `fcop://letter/zh` 或 `fcop/LETTER-TO-ADMIN.md`
